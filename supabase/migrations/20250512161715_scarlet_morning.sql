/*
  # Add missing product types to database schema
  
  1. Changes
     - Update the products table's type CHECK constraint to include 'physical', 'affiliate', 'service', 'ticket'
     - Add new columns for these product types
     - Update the validate_product_type function to handle the new types
...
ALTER TABLE products 
  ADD COLUMN IF NOT EXISTS ticket_type TEXT CHECK (ticket_type IN ('online', 'physical', 'hybrid')),
  ...
  ADD COLUMN IF NOT EXISTS ticket_delivery_method TEXT CHECK (ticket_delivery_method IN ('email', 'print', 'will_call', 'mobile')),
...
-- Update the validate_product_type function to handle new product types
